...
def conduct_experiment_latent(X, step_model_optimizer_loss, save_path, device='cpu', epochs=5000, save_iter=1000, print_iter=100):
    ...
    for i in range( 500):
        ...
        if torch.isnan(closure()):
            ...
            optimizer_BFGS = torch.optim.LBFGS(ode_trained.parameters(), lr=0.1,
                    history_size=10, 
                    max_iter=2)
        else:
            copy_net = copy.deepcopy(ode_trained)
...
def train_model(stuff):
    ...
    conduct_experiment_latent(X, model, path, device=device, epochs=30000, save_iter=10000, print_iter=5000)
