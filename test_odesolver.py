import matplotlib.pyplot as plt
import numpy as np
import torch
from nodepy import rk
from odesolver import RRK

# Harmonic Oscillator with Quartic Entropy/Energy
class HarmonicOscillator:
  def f(w):
      return torch.tensor([-w[1], w[0]])

  def eta(w):
      return torch.tensor([w[0]*w[0]*w[0]*w[0] + 2 * w[0]*w[0] * w[1]*w[1] + w[1]*w[1]*w[1]*w[1]])

  def deta(w):
      return torch.tensor([4*w[0]*w[0]*w[0] + 4*w[0]*w[1]*w[1], 4*w[1]*w[1]*w[1] + 4*w[1]*w[0]*w[0]])

  def u_analytical(t):
      w0 = torch.cos(t)
      w1 = torch.sin(t)
      return torch.tensor([w0, w1])


## Solve forward

print(' \n-- Solving forward --\n ')


u0 = torch.tensor([1., 0.])
dt = 0.9
t0 = torch.tensor([0.])
tf = torch.tensor([100.])

ssp3 = rk.loadRKM('SSP33').__num__()
ode_solver = RRK(h_max=dt, rkm=ssp3, relaxation=True, rescale_step=True)

tt, uu = ode_solver.solve(u0, t0, tf, HarmonicOscillator.f)

# print('t history = ', tt)

H = torch.cat([HarmonicOscillator.eta(uu[:,i]) for i in range(uu.shape[1])])
plt.plot(tt, H - H[0]);
plt.xlabel("$t$"); plt.ylabel("$\eta(u_\mathrm{num}(t)) - \eta(u_0)$"); plt.xlim(tt[0], tt[-1]);

print('u(t_0) = ', uu[:,0])
print('u(t_f) = ', uu[:,-1])
print("Error at tf: %.3e" % np.linalg.norm(uu[:,-1] - HarmonicOscillator.u_analytical(tt[-1])))
print("Pointwise mean error: %.3e" % np.mean( [np.linalg.norm(uu[:,i] - HarmonicOscillator.u_analytical(tt[i])) for i in range(tt.size(dim=0))] ))

print(' \n-- Solving backward --\n ')

## Solve backward

u0 = uu[:, -1]
tf = torch.tensor([0.])
t0 = torch.tensor([10.])

tt, uu = ode_solver.solve(u0, t0, tf, HarmonicOscillator.f)

# print('t history = ', tt)

H = torch.cat([HarmonicOscillator.eta(uu[:,i]) for i in range(uu.shape[1])])
plt.plot(tt, H - H[0]);
plt.xlabel("$t$"); plt.ylabel("$\eta(u_\mathrm{num}(t)) - \eta(u_0)$"); plt.xlim(tt[0], tt[-1]);

print('u(t_0) = ', uu[:,-1])
print('u(t_f) = ', uu[:,0])
print("Error at t0: %.3e" % np.linalg.norm(uu[:,-1] - HarmonicOscillator.u_analytical(tt[-1])))
print("Pointwise mean error: %.3e" % np.mean( [np.linalg.norm(uu[:,i] - HarmonicOscillator.u_analytical(tt[i])) for i in range(tt.size(dim=0))] ))
