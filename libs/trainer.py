...
class Trainer(eqx.Module):
    ...
    def loss_fn_mse(self, params, statics, batch, loss=True):
        ...
        error_grad = jnp.mean((t[(start - 1) : -1] / 10)*jnp.sqrt(\
                jnp.sum(((xhat[:,start:,0] - xhat[:,(start - 1) : -1,0]) / ts_del)** 2
                ) ))
    ...
    def return_loss_grad(self, params, static, batch):
        ...
        loss, dist, error_grad, yhat =self.loss_fn_mse(params, static, batch, loss=False)
    ...
    def train__EUC__(self, trainloader, params, static, optim, model_path, n_iter=1000, save_iter=200, print_iter=200):
        ...
        x0 = x[:,0,:]
        t=t.reshape([-1])
        config= {'int_step': init_step,\
            'N_Max_constraints': N_max_constraints,\
            'dist_flag': dist_flag, 'step': init_step}
        batch=(t, x0, x, config)
        opt_state = optim.init_state(init_params=params,\
        static=static, batch=batch)
        
        #--------------------------------------------------------
        pbar = tqdm(range(n_iter))
        for step in pbar:
            batch=(t, x0, x, {'int_step': init_step,\
            'N_Max_constraints': N_max_constraints,\
                'dist_flag': dist_flag, \
                'step': step})
            params, opt_state =  optim.update(params=params, state=opt_state,\
                static=static, batch= batch)
